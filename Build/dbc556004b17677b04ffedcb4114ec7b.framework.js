...
function Microphone_CreateSoundClipForRecording(channels, length, sampleRate) {
 ...
 soundClip.getData = function(ptr, length) {
  ...
  const output = GROWABLE_HEAP_F32().subarray(startOutputBuffer, startOutputBuffer + bufferLength);
...
function _JS_Sound_GetAudioBufferSampleRate(soundInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(48, 1, soundInstance);
 if (WEBAudio.audioWebEnabled == 0) return WEBAudio.FAKEMOD_SAMPLERATE;
...
function _JS_Sound_GetLoadState(bufferInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 1, bufferInstance);
...
function _JS_Sound_GetMetaData(bufferInstance, metaData) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(51, 1, bufferInstance, metaData);
 metaData = metaData >> 2;
...
function jsAudioCreateUncompressedSoundClipFromPCM(channels, length, sampleRate, ptr) {
 ...
 for (var i = 0; i < channels; i++) {
  ...
  var source = new Float32Array(GROWABLE_HEAP_F32().subarray(offs, offs + length));
...
function _JS_SystemInfo_GetCanvasClientSize(domElementSelector, outWidth, outHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(71, 1, domElementSelector, outWidth, outHeight);
...
function _JS_SystemInfo_GetScreenSize(outWidth, outHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(77, 1, outWidth, outHeight);
 outWidth = outWidth >> 3;
 outHeight = outHeight >> 3;
...
function _JS_SystemInfo_GetStreamingAssetsURL(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(78, 1, buffer, bufferSize);
...
function _JS_WebRequest_Abort(requestId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(89, 1, requestId);
...
function _JS_WebRequest_Create(url, method) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(90, 1, url, method);
...
function _JS_WebRequest_GetResponseMetaData(requestId, headerBuffer, headerSize, responseUrlBuffer, responseUrlSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(91, 1, requestId, headerBuffer, headerSize, responseUrlBuffer, responseUrlSize);
 var response = wr.responses[requestId];
 if (!response) {
...
function _JS_WebRequest_GetResponseMetaDataLengths(requestId, buffer) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(92, 1, requestId, buffer);
 buffer = buffer >> 2;
 var response = wr.responses[requestId];
 if (!response) {
...
function _JS_WebRequest_Release(requestId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(93, 1, requestId);
...
function _JS_WebRequest_Send(requestId, ptr, length, arg, onresponse, onprogress) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(94, 1, requestId, ptr, length, arg, onresponse, onprogress);
 ptr = ptr;
...
function _JS_WebRequest_SetRedirectLimit(request, redirectLimit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(95, 1, request, redirectLimit);
...
function _JS_WebRequest_SetRequestHeader(requestId, header, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(96, 1, requestId, header, value);
 var requestOptions = wr.requests[requestId];
 if (!requestOptions) {
  return;
 }
...
function _JS_WebRequest_SetTimeout(requestId, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(97, 1, requestId, timeout);
 var requestOptions = wr.requests[requestId];
 if (!requestOptions) {
  return;
 }
...
function pthreadCreateProxied(pthread_ptr, attr, startRoutine, arg) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(98, 1, pthread_ptr, attr, startRoutine, arg);
...
function ___syscall__newselect(nfds, readfds, writefds, exceptfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 1, nfds, readfds, writefds, exceptfds, timeout);
 try {
...
function ___syscall_accept4(fd, addr, addrlen, flags, d1, d2) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(100, 1, fd, addr, addrlen, flags, d1, d2);
 try {
...
function ___syscall_chmod(path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(101, 1, path, mode);
 try {
...
function ___syscall_faccessat(dirfd, path, amode, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(103, 1, dirfd, path, amode, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function ___syscall_fcntl64(fd, cmd, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(104, 1, fd, cmd, varargs);
...
function ___syscall_fstat64(fd, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(105, 1, fd, buf);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function ___syscall_getcwd(buf, size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(106, 1, buf, size);
 try {
  if (size === 0) return -28;
  var cwd = FS.cwd();
...
function ___syscall_getdents64(fd, dirp, count) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(107, 1, fd, dirp, count);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (!stream.getdents) {
...
function ___syscall_getsockopt(fd, level, optname, optval, optlen, d1) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(108, 1, fd, level, optname, optval, optlen, d1);
 try {
  var sock = getSocketFromFD(fd);
  if (level === 1) {
   if (optname === 4) {
...
function ___syscall_ioctl(fd, op, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(109, 1, fd, op, varargs);
 SYSCALLS.varargs = varargs;
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  switch (op) {
...
function ___syscall_lstat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(110, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_mkdirat(dirfd, path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(111, 1, dirfd, path, mode);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  path = PATH.normalize(path);
...
function ___syscall_newfstatat(dirfd, path, buf, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(112, 1, dirfd, path, buf, flags);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_openat(dirfd, path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(113, 1, dirfd, path, flags, varargs);
 SYSCALLS.varargs = varargs;
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(116, 1, olddirfd, oldpath, newdirfd, newpath);
 try {
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(117, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
  return 0;
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(159, 1, fd, offset, whence, newOffset);
 try {
...
function _navigator_gpu_request_adapter_async(options, adapterCallback, userData) {
 ...
 let gpu = navigator["gpu"], opts = options ? {
  "featureLevel": [ , "core", "compatibility" ][GROWABLE_HEAP_U32()[options]],
  "powerPreference": [ , "low-power", "high-performance" ][GROWABLE_HEAP_U32()[options + 1]],
  "forceFallbackAdapter": !!GROWABLE_HEAP_U32()[options + 2],
  "xrCompatible": !!GROWABLE_HEAP_U32()[options + 3]
 } : {};
 if (gpu) {
  function cb(adapter) {
...
function _wgpu_device_create_texture(device, descriptor) {
 ...
 let desc = {
  ...
  "textureBindingViewDimension": GPUTextureViewDimensions[GROWABLE_HEAP_U32()[descriptor + 11]]
 };
